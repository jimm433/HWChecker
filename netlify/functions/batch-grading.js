...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        // 解析請求數據
        const { submissions, gradingOptions } = JSON.parse(event.body);
        console.log(`批次批改 ${submissions.length} 份作業`);
...
`;
                ...
                console.error(`單個作業批改失敗 ${submission.id}:`, submissionError);
                return {
                    id: submission.id,
        ...
        return {
            ...
            body: JSON.stringify({
                success: false,
                message: error.message || '批次批改發生未知錯誤'
            })
        };
    }
};